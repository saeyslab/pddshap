...
class ResultReader:
    ...
    def __init__(self, data_dir: str, result_dir: str, model: str, baseline: str):
        ...
        self.datasets = [
            ds for ds in os.listdir(data_dir) if ds in os.listdir(result_dir)
        ]
    ...
    def _load_baseline(self, dataset: str) -> npt.NDArray:
        ...
        return np.load(
            os.path.join(
                self.data_dir, dataset, "shap", self.model, f"{self.baseline}.npy"
            )
        )
    ...
    def get_score(self, dataset: str, score="pearson") -> Dict[str, npt.NDArray]:
        ...
        for frac in pdd_values.keys():
            ...
            result[frac] = np.zeros(
                shape=(baseline_values.shape[0], baseline_values.shape[2])
            )
            ...
            for i in range(baseline_values.shape[0]):
                for j in range(baseline_values.shape[2]):
                    result[frac][i, j] = score_fn(
                        pdd_values[frac][i, :, j], baseline_values[i, :, j]
                    )
            """
            else:
    ...
    def get_runtimes(self, dataset: str) -> Dict:
        ...
        # Get baseline value
        bl_df = pd.read_csv(
            os.path.join(self.data_dir, dataset, "shap", self.model, "runtimes.csv")
        )
        baseline_values = self._load_baseline(dataset)
        num_rows = baseline_values.shape[0]
        # Extract the single entry for the baseline that we want, divide by number of rows
        result = {
            "baseline": bl_df.loc[bl_df["explainer"] == self.baseline][
                "runtime (s)"
            ].to_numpy()[0]
            / num_rows
        }
        ...
        for index, row in pdd_df.iterrows():
            ...
            result["pddshap"][frac] = {
                ...
                "inference": row["inference"] / num_rows,
            }
